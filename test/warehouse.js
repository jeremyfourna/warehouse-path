...
describe('warehouse.js', () => {
  describe('negativeValues', () => {
    it('return [-1] for [1, 2, -1, 3, 0]', () => {
      ...
      assert.isArray(negativeValues(test));
      assert.lengthOf(negativeValues(test), 1);
      assert.equal(R.nth(0, negativeValues(test)), [-1]);
    });
    it('return [] for []', () => {
      const test = [];

      assert.isArray(negativeValues(test));
      assert.lengthOf(negativeValues(test), 0);
    });
    it('return [-1, -2, -3, -4] for [-1, -2, -3, -4]', () => {
      const test = [-1, -2, -3, -4];

      assert.isArray(negativeValues(test));
      assert.lengthOf(negativeValues(test), 4);
      assert.equal(R.nth(0, negativeValues(test)), [-1]);
      assert.equal(R.nth(1, negativeValues(test)), [-2]);
      assert.equal(R.nth(2, negativeValues(test)), [-3]);
      assert.equal(R.nth(3, negativeValues(test)), [-4]);
    });
    ...
    it('return [-1, -4] for [-1, 2, -4]', () => {
      ...
      assert.isArray(negativeValues(test))
      assert.lengthOf(negativeValues(test), 2)
    });
  });
